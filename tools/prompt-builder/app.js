...
let activeDir='SE',activeFrame=0,reference={name:'',type:'',dataUrl:'',ready:false,error:''};
...
async function importReference(file){
  reference={name:file?.name||'',type:file?.type||'',dataUrl:'',ready:false,error:''};
  const status=$('referenceStatus');status.className='reference-status';status.textContent=file?`Referenz wird geladen… ${file.name}`:'Keine Referenz geladen.';$('pdf').disabled=true;
  if(!file){render();return}
  try{
    const blob=file instanceof Blob?file:new Blob([await file.arrayBuffer()],{type:file.type||'image/jpeg'});
    const dataUrl=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(reader.error||new Error('FileReader fehlgeschlagen'));reader.onabort=()=>reject(new Error('Einlesen abgebrochen'));reader.onload=()=>resolve(String(reader.result||''));reader.readAsDataURL(blob)});
    if(!dataUrl.startsWith('data:image/'))throw new Error('Datei wurde nicht als Bild erkannt');
    await new Promise((resolve,reject)=>{const img=new Image();img.onload=resolve;img.onerror=()=>reject(new Error('Bild konnte nicht dekodiert werden'));img.src=dataUrl});
    reference={name:file.name||'character-reference',type:file.type||'image/*',dataUrl,ready:true,error:''};
  }catch(err){reference={name:file.name||'',type:file.type||'',dataUrl:'',ready:false,error:err?.message||String(err)}}
  render();
}
let lastImportKey='';
function onReferenceSelection(e){const file=e.target?.files?.item?.(0)||e.target?.files?.[0]||null;if(!file){importReference(null);return}const key=`${file.name}|${file.size}|${file.lastModified}`;if(key===lastImportKey&&reference.ready)return;lastImportKey=key;importReference(file)}
$('referenceFile').addEventListener('change',onReferenceSelection);
$('referenceFile').addEventListener('input',onReferenceSelection);
...
$('json').onclick=()=>{const s=state();const a=anim();const pkg={...s,prompt:prompt(s),authoritativeReference:{...s.authoritativeReference,dataUrl:reference.dataUrl||null},directionSet:dirs,animationFrames:a.frames.map((f,i)=>({frame:i+1,...f})),generationMatrix:dirs.flatMap(d=>a.frames.map((f,i)=>({direction:d,frame:i+1,frameName:f.name,jobId:`${slug(s.character)}_${slug(s.animation)}_${d}_f${String(i+1).padStart(2,'0')}`})))};download(filename(s,'json'),JSON.stringify(pkg,null,2),'application/json;charset=utf-8')};
$('pdf').onclick=()=>{if(!reference.ready)return;window.print()};
